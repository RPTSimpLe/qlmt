...
namespace QuanLyMayTinh
{
    public partial class Form_ThongKeThu : Form
    {
        ...
        private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            g.DrawString("Ngày bán", font, brush, 710, yPos);

            yPos += cellHeight;

            DataTable dts = bUS_IncomeStatistics.getAll();
            ...
            for (int i = 0; i < dts.Rows.Count; i++)
            {
                ...
                g.DrawRectangle(pen, 460, yPos, 50, cellHeight);
                g.DrawString(dts.Rows[i]["Số lượng"].ToString(), font, brush, 460, yPos);

                g.DrawRectangle(pen, 510, yPos, cellWidth, cellHeight);
                g.DrawString(dts.Rows[i]["Giá"].ToString(), font, brush, 510, yPos);

                g.DrawRectangle(pen, 610, yPos, cellWidth, cellHeight);
                g.DrawString(dts.Rows[i]["Tổng"].ToString(), font, brush, 610, yPos);

                g.DrawRectangle(pen, 710, yPos, cellWidth, cellHeight);
                g.DrawString(dts.Rows[i]["Ngày bán"].ToString().Substring(0, 9), font, brush, 710, yPos);
